...
class AttentionNet(nn.Module):
    def __init__(self, n_output):
        ...
        self.conv4 = nn.Conv2d(256, 512, kernel_size=1, stride=1)
        self.conv5 = nn.Conv2d(256, 512, kernel_size=1, stride=1)
        self.alphalayer = nn.Conv2d(64, 1, kernel_size=1, stride=1)
        self.alphalayer2 = nn.Conv2d(512, 1, kernel_size=1, stride=1)
        self.unpool = nn.MaxUnpool2d(kernel_size=3, stride=2)
        self.fc1 = nn.Linear(72*72, 500)
        self.act1 = nn.ReLU()
        self.dp1 = nn.Dropout(p=0.2)
        self.fc2 = nn.Linear(500, n_output)

        # self.unpool_pad = nn.MaxUnpool2d(kernel_size=3, stride=2)
...
def mobilenet_v2_att(pretrained=False, progress=True, **kwargs):
    """
...
if __name__ == '__main__':
    model = AttentionNet()
    img = torch.ones(4, 3, 587, 587)
    x = model(img)
    summary(model, (3, 587, 587), device="cpu")